...
static const NSString * CSToastPendingToastKey  = @"CSToastPendingToastKey";
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)point {
    ...
    UIView *pendingToast = objc_getAssociatedObject(self, &CSToastPendingToastKey);
    if (pendingToast) {
    ...
    objc_setAssociatedObject(self, &CSToastPendingToastKey, toast, OBJC_ASSOCIATION_RETAIN);
    ...
    [UIView animateWithDuration:CSToastFadeDuration
                     ...
                     animations:^{
                         ...
                         if (pendingToast == toast) {
                             objc_setAssociatedObject(self, &CSToastPendingToastKey, nil, OBJC_ASSOCIATION_RETAIN);
                         }
                     }];
}
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    CGFloat wrapperWidth = ((longerLeft + longerWidth + CSToastHorizontalPadding) < imageWidth + (CSToastHorizontalPadding * 2)) ? imageWidth + (CSToastHorizontalPadding * 2) : (longerLeft + longerWidth + CSToastHorizontalPadding);
    CGFloat wrapperHeight = 0;
    // Use title instead of message bounds if there is no message
    if (messageLabel != nil) {
        wrapperHeight = ((messageTop + messageHeight + CSToastVerticalPadding) < imageHeight + (CSToastVerticalPadding * 2)) ? imageHeight + (CSToastVerticalPadding * 2) : (messageTop + messageHeight + CSToastVerticalPadding);
    }
    else {
        wrapperHeight = ((titleTop + titleHeight + CSToastVerticalPadding) < imageHeight + (CSToastVerticalPadding * 2)) ? imageHeight + (CSToastVerticalPadding * 2) : (titleTop + titleHeight + CSToastVerticalPadding);
    }
